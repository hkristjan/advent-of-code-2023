...
for (line=0; line<inputArray.length; line++) {
    ...
    for (char=0; char<inputArray.length; char++) {
        ...
        const prevLineChar = inputArray?.[line - 1]?.[char];
        const currLineChar = inputArray?.[line]?.[char];
        const nextLineChar = inputArray?.[line + 1]?.[char];

        //check for symbols
        if (
            isSymbol(prevLineChar) ||
            isSymbol(currLineChar) ||
            isSymbol(nextLineChar)
        ) {
            symbolMatch = true;
        ...
        if (
            ...
            if (symbolMatch && matchedNumber !== '') {
                console.log(`Matched number: ${matchedNumber}`);
                if (!symbolMatchThisColumn) {
                    symbolMatch = false;
                }
                sum += parseInt(matchedNumber);
            } else if (matchedNumber === '' && !symbolMatchThisColumn) {
                symbolMatch = false;
            }
            matchedNumber = "";
        } else {
            matchedNumber += currLineChar;
        }
    }
}
