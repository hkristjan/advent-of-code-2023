...
const gamesObject = gamesArray.reduce((acc, el) => {
    const [game, colors] = el.split(':');
    const colorsArray = colors.split(';').map(
        (el) => el.trim().split(',').reduce((acc, el) => {
            const [quantity, color] = el.trim().split(' ');
            acc[color.trim()] = parseInt(quantity.trim());
            return acc;
        }, {})
    );

    acc[game.trim()] = colorsArray;
    return acc;
}, {});

Object.entries(gamesObject).forEach(([game, rounds]) => {
    for (round of rounds) {
        for (color in round) {
            if ((config[color] ?? 0) < round[color]) {
                console.log(`(config[color] ?? 0) < round[color]`);
                console.log(`config:${config[color]} game:${round[color]}`);
...
console.log(`Sum of all valid games is ${sum}`);
