...
function isSymbol (char) {
    ...
    return !char?.match(/\d+|\./)?.length;
}

function getNumber (line, charIndex, direction = 1) {
    const matchingNumber = [];

    while (!isNaN(inputArray?.[line]?.[charIndex + direction])) {
        charIndex += direction;
        matchingNumber.push(inputArray?.[line]?.[charIndex]);
    }

    return direction > 0 ? matchingNumber : matchingNumber.reverse();
}

function getMatches (charIndex, line) {
    const matchingNumbers = [];

    const isOnSpot = !isNaN(inputArray?.[line]?.[charIndex]);
    const isLeft = !isOnSpot && !isNaN(inputArray?.[line]?.[charIndex - 1]);
    const isRight = !isOnSpot && !isNaN(inputArray?.[line]?.[charIndex + 1]);

    if (isOnSpot) {
        matchingNumbers.push([
            ...getNumber(line, charIndex, -1),
            inputArray[line][charIndex],
            ...getNumber(line, charIndex),
        ]);
    }

    if (isLeft) {
        matchingNumbers.push([...getNumber(line, charIndex, -1)]);
    }

    if (isRight) {
        matchingNumbers.push([...getNumber(line, charIndex)]);
    }

    return matchingNumbers.map(item => parseInt(item.join('')));
}
...
for (line=0; line<inputArray.length; line++) {
    ...
    for (char=0; char<inputArray.length; char++) {
        let symbolMatchThisColumn = false;
        const currLineChar = inputArray?.[line]?.[char];

        //check for symbols
        if (
            isSymbol(currLineChar)
        ) {
            symbolMatch = true;
            symbolMatchThisColumn = true;
            let matches = [];

            console.log(`Matched symbol: ${currLineChar}`)

            matches = [...matches, ...getMatches(char, line - 1)];
            matches = [...matches, ...getMatches(char, line)];
            matches = [...matches, ...getMatches(char, line + 1)];
