...
for (word of values) {
    ...
    for (wordDigit in mappings) {
        let replacedWord = wordDigit.split("");
        replacedWord.splice(2, 0, `${mappings[wordDigit]}`);
        replacedWord = replacedWord.join("");
        console.log(replacedWord);
        word = word.replaceAll(wordDigit, replacedWord);
    }
    console.log("replaced", word);

    const match = word.match(/^.*?(\d).*(\d).*$|^.*?(\d).*$/i);
    if (match[3] ?? false) {
        number = parseInt(`${match[3]}${match[3]}`);
    } else {
